...
class PGSQLExporter:
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                for email in emails:
                    ...
                    for listname in email.list_names:
                        listid = self.__getListIdFromName(cursor, select_list_id, listname)
                        if not listid:
                            continue
                        cursor.execute(insert_message_list, (email.message_id, listid[0],))
    ...
    def addReferences(self, emails):
        try:
            with self.connection.cursor() as cursor:
                insert_reference = "INSERT INTO luath_api_message_references (from_message_id, to_message_id) VALUES %s"

                for email in emails:
                    totalString = "("
                    for reference in email.references:
                        totalString += "(%s,%s)," % (email.message_id, reference)
                    totalString += ")"
                    try:
                        cursor.execute(insert_reference, (totalString))
                        self.connection.commit()
    ...
    def __containsEmail(self, cursor, sql, email):
        cursor.execute(sql, (email.message_id,))
